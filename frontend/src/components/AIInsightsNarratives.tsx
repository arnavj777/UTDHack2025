...
import { Sparkles, TrendingUp, TrendingDown, AlertCircle, Download, RefreshCw, Plus, Edit, Trash2 } from 'lucide-react';
...
export function AIInsightsNarratives() {
  ...
  const loadAIInsights = async () => {
    try {
      setLoading(true);
      setError('');
      const data = await aiInsightService.list();
      setAiInsights(data);
    } catch (err) {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this AI insight?')) return;
    try {
      await aiInsightService.delete(id);
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-2">
          <Button className="gap-2" onClick={() => navigate('/workspace/ai-insights/create')}>
      ...
      {aiInsights.length > 0 && (
        <Card className="p-6">
          <h3 className="mb-4">Saved AI Insights</h3>
          {loading ? (
            ...
            <div className="space-y-3">
              {aiInsights.map((insight) => (
                <div key={insight.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-slate-50">
                  <div className="flex-1">
                    <h4 className="font-medium">{insight.title}</h4>
                    {insight.description && <p className="text-slate-600 text-sm mt-1">{insight.description}</p>}
                    <div className="flex gap-2 mt-2">
                      <Badge variant="outline">{insight.insight_type || 'analysis'}</Badge>
                      {insight.confidence_score !== undefined && (
                        <span className="text-slate-600 text-sm">Confidence: {insight.confidence_score}%</span>
                      )}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={() => navigate(`/workspace/ai-insights/edit/${insight.id}`)}>
                      <Edit className="w-4 h-4 mr-1" />
                      Edit
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => handleDelete(insight.id)}>
