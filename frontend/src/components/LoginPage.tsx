...
export function LoginPage() {
  ...
  // Check if user is already authenticated (e.g., from OAuth redirect)
  useEffect(() => {
    if (!authLoading && user) {
      ...
      <Card className="w-full max-w-md p-8">
        ...
        <div className="space-y-3">
          <Button 
            ...
            onClick={async () => {
              try {
                const url = await authService.getOAuthLoginUrl('google');
          ...
          <Button 
            ...
            onClick={async () => {
              try {
                const url = await authService.getOAuthLoginUrl('github');
                window.location.href = url;
              } catch (error) {
