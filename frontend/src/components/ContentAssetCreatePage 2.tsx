...
export function ContentAssetCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await contentAssetService.create(formData);
      navigate('/workspace/content-automation');
    } catch (err) {
      if (err instanceof ApiError) {
        setError(err.message);
      } else {
        setError('Failed to create content asset. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h1 className="mb-2">Create Content Asset</h1>
        <p className="text-slate-600">Add a new content asset</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3">
            <Button type="submit" disabled={loading || !formData.title}>
              {loading ? 'Creating...' : 'Create Content Asset'}
            </Button>
