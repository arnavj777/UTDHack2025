...
export function MetricEditPage() {
  ...
  useEffect(() => {
    const loadMetric = async () => {
      ...
      try {
        setLoadingData(true);
        const metric = await metricService.get(parseInt(id));
        setFormData({
          title: metric.title || '',
          description: metric.description || '',
          metric_type: metric.metric_type || 'kpi',
        ...
        if (err instanceof ApiError) {
          ...
          setError('Failed to load metric. Please try again.');
        }
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await metricService.update(parseInt(id), formData);
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h1 className="mb-2">Edit Metric</h1>
        <p className="text-slate-600">Update metric</p>
      </div>
